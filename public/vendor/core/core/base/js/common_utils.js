...
$(document).ready(function () {
    ...
    if (params.user_navigation) {
        if (params.dls == "1" && $('.nav-tabs li.nav-item').eq(1).length) {
            $('.nav-tabs li.nav-item').eq(1).find('a').trigger('click');
        } else {
            $('.nav-tabs li.nav-item').eq(0).find('a').trigger('click');
        }
        $('.user-profile .crop-avatar').hide();
    }
    $('.role-container-loader .fa-spinner').hide();
        ...
        $.each(data, function (key, value) {
            ...
            if ($.inArray(type, excludeInputType) === -1) {
                if (type == 'radio' || type == 'checkbox') {
                 ...
                 else if ($elem.is('.datepicker,#dob,#expiry_date')) {
