...
let chartUtils = {
    ...
    plotChart: function (config) {
        ...
        let title = chartElm.replace("pie", "");
        title = title.replace(/\-/g, " ");
        $('#'+chartElm).find('.stats_header').html(title);
        $('#'+chartElm).find('.dashboard-chart-total').html(total);
...
setTimeout(function() {
    if (typeof(dashboardStatsConfig) != "undefined" && !$.isEmptyObject(dashboardStatsConfig)) {
        $.map(dashboardStatsConfig, function (obj, k) {
            if (obj.type == "table") {
                let widgetRoute = (obj.statsConfig && obj.statsConfig.route) ? obj.statsConfig.route : "crud.widget.dashboard-stats";console.log(route)
                BDashboard.loadWidget($("#" + k).find(".widget-content"), route(widgetRoute), obj);
            } else {
                let config = {
                    chartElm: k,
