...
let userUtils = {
    ...
    formAction: $(".main-form").length
        ? $(".main-form").parents("form:first").attr("action")
        : $('form[action$="postdata"]').attr("action"),
    init: function () {
        var table = $("#plugins-user-table").length
            ? $("#plugins-user-table").DataTable()
            : null;
        $(document).on(
            "click",
            'a[href^="users/viewdetail"],a[href*="users/edit"]',
            function (e) {
                e.preventDefault();
        ...
        $(document)
            .find('[name*="is_current_job"]:checked')
            .each(function () {
                let parent = $(this).parents(".grouppedLayout:first");
                parent.find('[name*="end_date"]').attr("disabled", "disabled");
                return;
        ...
        if (!currentLocation.includes("edit-profile")) {
            ...
            $(document)
                .find('input[name*="curriculum_vitae"]')
                .attr("disabled", true);
        }
        ...
        if (userUtils.formEl && !$("input[name='id']").val()) {
            userUtils.formEl
                .find('[name="user_addresses[present_country]"]')
                .val(defaultCountryId)
                .trigger("change");
        }
    },
    ...
    initGoogleMap: function () {
        ...
        if (!$.isEmptyObject(formData)) {
            ...
            presentLat = defaultLat;
            presentLng = defaultLng;
            permanentLat = defaultLat;
            permanentLng = defaultLng;
            nextKinLat = defaultLat;
            nextKinLng = defaultLng;
        }
        let googleMap = {
            displayError: false,
            viewOnlyMode: true,
        ...
        userUtils.presentLocationMap = $(
            ".present_address_block"
        ).customGoogleMap(googleMap);
        googleMap.addressPrefix = "permanent";
        googleMap.lat = permanentLat;
        googleMap.lng = permanentLng;
        userUtils.permanentLocationMap = $(
            ".permanent_address_block"
        ).customGoogleMap(googleMap);
        googleMap.addressPrefix = "";
        googleMap.submodule = "next_kin_details";
        googleMap.lat = nextKinLat;
        googleMap.lng = nextKinLng;
        userUtils.nextKinLocationMap =
            $(".next_kin_details").customGoogleMap(googleMap);
    },
    validateCurriculumVitae: function () {
        let valid = true;
        $("#wizard-step-p-4 .subFormRepeater > .form-group").each(function () {
            $(this).find(".customError").remove();

            $(this)
                .find('[name^="experience_infos"]+ .attachment-details')
                .each(function () {
                    let targetElm = $(this);
                    ...
                    if (
                        filePath &&
                        !inpElm.attr("disabled") &&
                        !allowedExtensions.exec(filePath)
                    ) {
                        $(
                            '<span class="invalid-feedback customError" style="display: inline;">The curriculum vitae field must be a "pdf,doc,docx,txt".</span>'
                        ).insertAfter($(targetElm).next());
                        valid = false;
                    }
                });
        });
        ...
        $.ajax({
            ...
            success: (response) => {
                if (response && !response.error) {
                    ...
                    let updateAction = userUtils.formAction.replace(
                        "postdata",
                    ...
                    $(el)
                        ...
                        .find(".required-field")
                        .append(
                            ' <a id="changeIdentification">Change ' +
                                label.replace("*", "") +
                                "</a>"
                        );
...
$(document).ready(function () {
    userUtils.init();
    if (window.location.href.indexOf("ivp") > -1) {
        // alert("your url contains the name ivp");
        $( ".widget-title" ).remove();
    }
});
