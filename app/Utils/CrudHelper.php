...
//            $result = \RvMedia::handleUpload($file, 0, 'website');
    ...
    public static function updateCoreUser($id, $request, $userRepository, $service, $activateUserService = null)
    {
        ...
        if (
            ($currentUser->hasPermission('users.update-profile') && $currentUser->getKey() === $user->id) ||
    ...
    public static function sendEmailConfig($moduleName, $emailConfigs, $data)
    {
        ...
        $recieverEmails = $emailIds = $username = [];
        $userId = "";
...
//        }
    ...
    public static function applyFilterCondition($repository, $query, string $key, string $operator, ?string $value)
    {
        ...
        switch ($key) {
            ...
            default:
                ...
                if (
                    in_array($table, config('general.entity_filter_supported_models')) ||
    ...
    public function getMSMECandidateDetails(Request $request, BaseHttpResponse $response)
    {
        ...
        if ($request->has('canditateId') && $request->has('scheme')) {
            ...
            if ($candidateDetails) {
                if (isset($candidateDetails->error_code) && isset($candidateDetails->error_desc)) {
                    $error_message = "Error occured. Code: " . $candidateDetails->error_code . ". Description: " . $candidateDetails->error_desc;
                    return $response->setError(true)
                        ->setMessage($error_message);
                } else {
                    $candidateInfo = (isset($candidateDetails->candidate)) ? $candidateDetails->candidate[0] : $candidateDetails;

                    $candidateName = (isset($candidateInfo->applicant_name)) ? $candidateInfo->applicant_name : (isset($candidateInfo->candidate_name) ? $candidateInfo->candidate_name : "");
                    $fatherName = "";
                    ...
                    if ($entrepreneur) {
                        ...
                        if ($trainee) {
                            ...
                            $return_data['message'] = $candidateName . " you are already enrolled";
                            //return $response->setError(true)
                    ...
                    $msmeDetails = [
                        'candidate_name' => (isset($candidateInfo->applicant_name)) ? $candidateInfo->applicant_name : (isset($candidateInfo->candidate_name) ? $candidateInfo->candidate_name : ""),
...
JOIN `mentees` `ME` ON `ME`.`entrepreneur_id` = `EE`.`id`)) AS `cnt`");
    ...
    */
    ...
    function getAcademicOptions(Request $request, BaseHttpResponse $response)
    {
        ...
        $config = array(
            'hub_institution_id' => array('fields' => ['spoke_registration.id', 'name AS text'], 'table' => 'spoke_registration', 'cndn' => ['spoke_registration.hub_institution_id' => $request->input('hub_institution_id')]),
