...
namespace Impiger\CustomField\Support;
...
class CustomFieldSupport
{
     ...
     * @return array
     */
    public function exportCustomFieldsData($morphClass, $morphId,$ignoreRules = false): array
    {
        ...
        foreach ($fieldGroups as $row) {
            if ($this->checkRules(json_decode($row->rules, true)) || $ignoreRules) {
                $result[] = [
                    'id'    => $row->id,
                    'title' => $row->title,
