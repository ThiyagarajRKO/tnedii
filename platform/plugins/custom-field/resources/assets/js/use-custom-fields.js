                ...
                config = $.extend(
                    true,
                    {
                        ...
                        filebrowserImageBrowseUrl:
                            RV_MEDIA_URL.base +
                            "?media-action=select-files&method=ckeditor&type=image",
                        filebrowserImageUploadUrl:
                            RV_MEDIA_URL.media_upload_from_editor +
                            "?method=ckeditor&type=image&_token=" +
...
class UseCustomFields {
    constructor() {
        ...
        this.CURRENT_DATA = Helpers.jsonDecode(
            this.base64Helper().decode(this.$_EXPORT_TO.text()),
            []
        );
    ...
    handleCustomFields() {
        ...
        let FIELD_TEMPLATE = {
            ...
            globalSkeleton: $(
                "#_render_custom_field_global_skeleton_template"
            ).html(),
            ...
            repeaterItem: $(
                "#_render_custom_field_repeater_item_template"
            ).html(),
            repeaterFieldLine: $(
                "#_render_custom_field_repeater_line_template"
            ).html(),
        };
        ...
        let initCustomFieldsBoxes = (boxes, $appendTo) => {
            boxes.forEach((box) => {
                ...
                if (box.type === "wysiwyg") {
                    initWYSIWYG(
                        $skeleton.find(".meta-box-wrap .wysiwyg-editor"),
                        box.options.wysiwygToolbar || "basic"
                    );
                }
            });
        };
        ...
        let registerLine = (box) => {
            let result = FIELD_TEMPLATE[box.type],
                $wrapper = $(
                    '<div class="lcf-' + box.type + '-wrapper"></div>'
                );
            ...
            switch (box.type) {
                ...
                case "password":
                    result = result.replace(
                        /__placeholderText__/gi,
                        box.options.placeholderText || ""
                    );
                    result = result.replace(
                        /__value__/gi,
                        box.value || box.options.defaultValue || ""
                    );
                    break;
                case "textarea":
                    result = result.replace(
                ...
                case "image":
                    ...
                    if (!box.value) {
                        let defaultImage = $(result)
                            .find("img")
                            .attr("data-default");
                        result = result.replace(
                            /__image__/gi,
                            defaultImage || box.options.defaultValue || ""
                        );
                    } else {
                        result = result.replace(
                            /__image__/gi,
                            box.thumb || box.options.defaultValue || ""
                        );
                    }
                    break;
                case "file":
                    ...
                    result = result.replace(
                        /__url__/gi,
                        box.full_url || box.options.defaultValue || ""
                    );
                    break;
                case "select":
                    $result = $(result);
                    ...
                    $result.val(
                        Helpers.arrayGet(box, "value", box.options.defaultValue)
                    );
                ...
                case "radio":
                    ...
                    choices.forEach((choice) => {
                        ...
                        template = template.replace(
                            /__id__/gi,
                            box.id + box.slug + repeaterFieldAdded
                        );
                        template = template.replace(
                            /__title__/gi,
                            choice[1] || ""
                ...
                case "repeater":
                    ...
                    $result
                        .find("> .repeater-add-new-field")
                        .html(box.options.buttonLabel || "Add new item");
                    $result.find("> .sortable-wrapper").sortable();
                    registerRepeaterItem(
                        box.items,
                        box.value || [],
                        $result.find("> .field-group-items")
                    );
                    return $result;
                case "wysiwyg":
        ...
        let registerRepeaterItem = (items, data, $appendTo) => {
            ...
            data.forEach((dataItem) => {
                let indexCss =
                    $appendTo.find("> .ui-sortable-handle").length + 1;
                ...
                registerRepeaterFieldLine(
                    items,
                    dataItem,
                    $result.find("> .field-line-wrapper > .field-group")
                );
        ...
        let registerRepeaterFieldLine = (items, data, $appendTo) => {
            data.forEach((item) => {
                ...
                if (item.type === "wysiwyg") {
                    initWYSIWYG(
                        $result.find("> .repeater-item-input .wysiwyg-editor"),
                        item.options.wysiwygToolbar || "basic"
                    );
                }
            });
            return $appendTo;
        ...
        this.$body.on("click", ".repeater-add-new-field", (event) => {
            event.preventDefault();
            let $groupWrapper = $.extend(
                true,
                {},
                $(event.currentTarget).prev(".field-group-items")
            );
        ...
        this.CURRENT_DATA.forEach((group) => {
            ...
            initCustomFieldsBoxes(
                group.items,
                $groupTemplate.find(".meta-boxes-body")
            );
    ...
    exportData() {
        ...
        let getFieldGroups = () => {
            ...
            $("#custom_fields_container")
                ...
                .each((index, el) => {
                    ...
                    let $items = $current.find(
                        "> .meta-boxes-body > .meta-box"
                    );
        ...
        let getFieldItemValue = ($item) => {
            let customFieldData = $.extend(
                true,
                {},
                $item.data("lcf-registered-data")
            );
            switch (customFieldData.type) {
                ...
                case "image":
                case "file":
                    customFieldData.value = $item
                        .find("> .meta-box-wrap input")
                        .val();
                    break;
                case "wysiwyg":
                    customFieldData.value = Helpers.wysiwygGetContent(
                        $item.find("> .meta-box-wrap textarea")
                    );
                    break;
                case "textarea":
                    customFieldData.value = $item
                        .find("> .meta-box-wrap textarea")
                        .val();
                    break;
                case "checkbox":
                    customFieldData.value = [];
                    $item
                        .find("> .meta-box-wrap input:checked")
                        .each((index, el) => {
                ...
                case "select":
                    customFieldData.value = $item
                        .find("> .meta-box-wrap select")
                        .val();
                    break;
                case "repeater":
                    customFieldData.value = [];
                    let $repeaterItems = $item.find(
                        "> .meta-box-wrap > .lcf-repeater > .field-group-items > li"
                    );
                    $repeaterItems.each((index, el) => {
                        let $current = $(el);
                        let fieldGroup = $current.find(
                            "> .field-line-wrapper > .field-group"
                        );
                        customFieldData.value.push(
                            getRepeaterItemData(fieldGroup.find("> li"))
                        );
                    });
                    break;
                default:
        ...
        let getRepeaterItemValue = ($item) => {
            ...
            switch (customFieldData.type) {
                ...
                case "file":
                    customFieldData.value = $item
                        .find("> .repeater-item-input input")
                        .val();
                    break;
                case "wysiwyg":
                    customFieldData.value = Helpers.wysiwygGetContent(
                        $item.find(
                            "> .repeater-item-input > .lcf-wysiwyg-wrapper > .wysiwyg-editor"
                        )
                    );
                    break;
                case "textarea":
                    customFieldData.value = $item
                        .find("> .repeater-item-input textarea")
                        .val();
                    break;
                case "checkbox":
                    customFieldData.value = [];
                    $item
                        .find("> .repeater-item-input input:checked")
                        .each((index, el) => {
                            customFieldData.value.push($(el).val());
                        });
                    break;
                ...
                case "select":
                    customFieldData.value = $item
                        .find("> .repeater-item-input select")
                        .val();
                    break;
                case "repeater":
                    customFieldData.value = [];
                    let $repeaterItems = $item.find(
                        "> .repeater-item-input > .lcf-repeater > .field-group-items > li"
                    );
                    $repeaterItems.each((index, el) => {
                        let $current = $(el);
                        let fieldGroup = $current.find(
