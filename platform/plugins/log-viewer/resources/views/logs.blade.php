                    ...
                    @foreach($rows as $date => $item)
                        <tr>
                            ...
                            <td class="text-right" style="width: 150px;">
                                <a href="{{ route('log-viewer::logs.show', [$date]) }}" id="tooltip" title="Search"class="btn btn-xs btn-info">
                                    <i class="fa fa-search"></i>
                                </a>
                                <a href="{{ route('log-viewer::logs.download', [$date]) }}" title="Download"class="btn btn-xs btn-success">
                                    <i class="fa fa-download"></i>
                                </a>
                               @if (Auth::user()->hasPermission('logs.destroy')) 
                                           
                                <a href="#delete-log-modal" title="Delete"class="btn btn-xs btn-danger" data-log-date="{{ $date }}">
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.result === 'success') {
                            ...
                            Impiger.showError('AJAX ERROR ! Check the console !');
                            console.error(data);
                        }
