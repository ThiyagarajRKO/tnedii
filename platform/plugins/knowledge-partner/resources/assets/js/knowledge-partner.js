class KnowledgePartnerPluginManagement {
    init() {
        ...
        $(document).on("click", ".answer-send-button", (event) => {
            ...
            if (typeof tinymce != "undefined") {
                ...
                typeof CKEDITOR.instances["message"] !== "undefined"
            ) {
            ...
            $.ajax({
                type: "POST",
                cache: false,
                url: route("knowledge-partners.reply", $("#input_knowledge_partner_id").val()),
                data: {
                    message: message,
                },
                success: (res) => {
                    if (!res.error) {
                        ...
                        $("#reply-wrapper").load(
                            window.location.href + " #reply-wrapper > *"
                        );
                    }
...
$(document).ready(() => {
    new KnowledgePartnerPluginManagement().init();
});
