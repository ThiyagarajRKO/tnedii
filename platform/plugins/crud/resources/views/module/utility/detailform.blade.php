        ...
        <thead>
			<tr>
				...
				<?php foreach ($tableGrid as $t) :
					if($t['view'] =='1'):
						echo '<th align="'.$t['align'].'">'.activeLang($t['label'],(isset($t['language'])? $t['language'] : array())).'</th>';
					endif;
				endforeach; ?>
				<th width="70"></th>
			  </tr>
        ...
        <tbody>
			...
			<tr>
				...
				<td >
					...
					onclick="ajaxInlineSave('#{{ $pageModule }}','{{url($pageModule.'/quicksave') }}','{{url($pageModule.'/detailview/form?md='.$filtermd) }}')">
					...
					<td width="50"> <?php echo $n;?>  </td>
					 ...
					 <?php foreach ($tableGrid as $field) :
					  ...
					  <td>
					  	...
					  	<a href="javascript:void(0)" class=" btn btn-sm btn-info" onclick="ajaxInlineEdit('#{{ $pageModule }}-{{$n}}');" 	><i class="fa fa-pencil"></i></a>
					  	<?php endif;
					  	if($access['is_remove'] ==1) :?>					  	
					  	<a href="javascript:void(0)" onclick="ajaxInlineRemove('#{{ $pageModule }}-{{$n}}','{{url($pageModule."/removerow/".$row->$primarykey) }}')" class=" quickRemove btn btn-sm btn-danger" ><i class="fa fa-minus"></i></a>
					  	<?php endif; ?>
					  </td>			 
                </tr>
