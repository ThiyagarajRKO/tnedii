...
use Impiger\Language\Facades\LanguageFacade;
use Impiger\Language\Http\Middleware\LocaleSessionRedirect;
use Impiger\Language\Http\Middleware\LocalizationRedirectFilter;
use Impiger\Language\Http\Middleware\LocalizationRoutes;
use Impiger\Language\Models\Language as LanguageModel;
use Impiger\Language\Models\LanguageMeta;
use Impiger\Language\Repositories\Caches\LanguageCacheDecorator;
use Impiger\Language\Repositories\Caches\LanguageMetaCacheDecorator;
use Impiger\Language\Repositories\Eloquent\LanguageMetaRepository;
use Impiger\Language\Repositories\Eloquent\LanguageRepository;
...
class LanguageServiceProvider extends ServiceProvider
{
     ...
     * @return array
     * @since 2.2
     * @customized vijayaragavan ambalam
     */
    public function addLanguageSwitcherToTable($buttons, $model)
    {
        if (in_array($model, Language::supportedModels())) {
            ...
            $currentMultidomain = null;
            if (is_plugin_active('multidomain')) {
                $currentMultidomain = Multidomain::getCurrentAdminDomainId();
            }
            foreach ($activeLanguages as $item) {
                if(!empty($currentMultidomain))
                {
                    $languageButtons[] = [
                        'className' => 'change-data-language-item ' . ($item->lang_code == $currentLanguage ? 'active' : ''),
                        'text'      => Html::tag('span', $item->lang_name,
                            ['data-href' => route('languages.change.data.language', ['ref_lang'=>$item->lang_code, 'ref_domain'=>$currentMultidomain])])->toHtml(),
                    ];
                }
                else
                {
            ...
            if(!empty($currentMultidomain))
            {
                $languageButtons[] = [
                    ...
                    'text'      => Html::tag('span', trans('plugins/language::language.show_all'),
                        ['data-href' => route('languages.change.data.language', ['ref_lang'=>'all', 'ref_domain'=>$currentMultidomain])])->toHtml(),
                ];
            }
            else
            {
     ...
     */
    public function saveLanguageMeta($referenceId,$model){
            $currentLanguage = Language::getCurrentAdminLocaleCode();
            $data = [
            'reference_id' => $referenceId,
            'reference_type' => $model,
            'lang_meta_code' => $currentLanguage,
