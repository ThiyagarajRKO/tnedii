...
namespace Impiger\Language\Http\Controllers;
...
use Impiger\Base\Supports\Language;
use Impiger\Language\Http\Requests\LanguageRequest;
...
class LanguageController extends BaseController
{
     ...
     */
    public function getChangeDataLanguage($ref_lang, $ref_domain=null, LanguageManager $language)
    {
        ...
        if(!empty($ref_domain))
        {
            if ($ref_lang !== $language->getDefaultLocaleCode()) {
                $queryString = '?' . http_build_query(['ref_lang' => $ref_lang, 'ref_domain' => $ref_domain]);
            }
            else
            {
                $queryString = '?' . http_build_query(['ref_domain' => $ref_domain]);
            }
        }
        else
        {
            if ($ref_lang !== $language->getDefaultLocaleCode()) {
                $queryString = '?' . http_build_query(['ref_lang' => $ref_lang]);
            }
        }
