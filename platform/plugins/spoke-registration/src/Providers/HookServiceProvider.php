    ...
    public function boot(FormBuilder $formBuilder)
    {
        ...
        $this->app->booted(function () use ($formBuilder) {
            ...
            if (function_exists('add_shortcode')) {
                ...
                $that = $this;
                CrudHelper::declareShortCode($that, 'spoke-registration');
            }
        });
    }

    public function addCrudStatsWidget($widgets, $widgetSettings) {
                return CrudHelper::addCrudStatsWidget('spoke-registration',$widgets, $widgetSettings);
            }
        ...
        if (defined('THEME_OPTIONS_MODULE_SCREEN_NAME')) {
            $this->app->booted(function () {
                Theme::asset()
                    ->container('footer')
                    ->usePath(false)
                    ->add('spoke-registration-custom-js', asset('vendor/core/plugins/spoke-registration/js/spoke-registration.js'), ['jquery'], [], '1.0.0');
            });
        }
    }
