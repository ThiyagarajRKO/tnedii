...
class QualificationsTable extends TableAbstract
{
    ...
    protected $editPermissions = "qualifications.edit";
    protected $deletePermissions = "qualifications.destroy";
    /* @customized by Sabari Shankar.Parthiban*/
    protected $printPreview = 'base.print';
    /**
     * QualificationsTable constructor.
     * @param DataTables $table
     * @param UrlGenerator $urlGenerator
     * @param QualificationsInterface $qualificationsRepository
     */
    public function __construct(DataTables $table, UrlGenerator $urlGenerator, QualificationsInterface $qualificationsRepository)
    {
        $this->repository = $qualificationsRepository;
        $this->setOption('id', 'plugins-qualifications-table');
    ...
    public function ajax()
    {
        $data = $this->table
            ...
            ->editColumn('name', function ($item) {
                ...
                #{hideOnEditLink}
                return CrudHelper::getNameFieldLink($item, 'qualifications', $isEdit, $isPublic);
            })
            ...
            ->addColumn('operations', function ($item) {
                ...
                #{hideOnEditAction}
                #{hideOnDeleteAction}
                return $this->getOperations($this->editPermissions, $this->deletePermissions, $item, "<a data-fancybox data-type='ajax' data-src='qualifications/viewdetail/$item->id ' href='javascript:void(0);' class='btn btn-icon btn-sm btn-primary' data-toggle='tooltip' data-original-title='View'><i class='fa fa-eye'></i></a>");
            });
    ...
    public function buttons()
    {
        ...
        $buttons = $this->addCreateButton(route('qualifications.create'), 'qualifications.create');


        return apply_filters(BASE_FILTER_TABLE_BUTTONS, $buttons, Qualifications::class);
    }
    ...
    {
        if (Auth::user() && !Auth::user()->hasPermission('qualifications.edit') ) {
                    return [];
                }
        return $this->addDeleteAction(route('qualifications.deletes'), 'qualifications.destroy', parent::bulkActions());
    }
    ...
    {
        return CrudHelper::getBulkChanges('qualifications', $isFilter, );
    }
    ...
    public function getDefaultButtons(): array
    {
        ...
        if (Auth::user() && Auth::user()->hasPermission('qualifications.export')) {
            $defaultBtns = array_merge($defaultBtns, ['export']);
        }

        if (Auth::user() && Auth::user()->hasPermission('qualifications.print')) {
    ...
    protected function checkDefault($item){
        if( isset($item->is_default) && $item->is_default){
            if(Auth::user() && (Auth::user()->is_admin || Auth::user()->isSuperUser())){
                $this->editPermissions = 'qualifications.edit';
            }else{
