...
class RegionTable extends TableAbstract
{
    ...
    protected $editPermissions = "region.edit";
    protected $deletePermissions = "region.destroy";
    ...
    public function __construct(DataTables $table, UrlGenerator $urlGenerator, RegionInterface $regionRepository)
    {
        ...
        $this->setOption('id', 'plugins-region-table');
    ...
    public function ajax()
    {
        $data = $this->table
            ...
            ->editColumn('name', function ($item) {
                ...
                #{hideOnEditLink}
                return CrudHelper::getNameFieldLink($item, 'region', $isEdit, $isPublic);
            })
            ...
            ->addColumn('operations', function ($item) {
                ...
                #{hideOnEditAction}
                #{hideOnDeleteAction}
                return $this->getOperations($this->editPermissions, $this->deletePermissions, $item, "<a data-fancybox data-type='ajax' data-src='regions/viewdetail/$item->id ' href='javascript:void(0);' class='btn btn-icon btn-sm btn-primary' data-toggle='tooltip' data-original-title='View'><i class='fa fa-eye'></i></a>".CrudHelper::getRowActivationActionBtn($item,'Impiger\MasterDetail\Models\Region', 'region.enable_disable',''));
            });
    ...
    {
        if (Auth::user() && !Auth::user()->hasPermission('region.edit') ) {
                    return [];
                }
    ...
    {
        return CrudHelper::getBulkChanges('region', $isFilter, );
    }
    ...
    public function getDefaultButtons(): array
    {
        ...
        if (Auth::user() && Auth::user()->hasPermission('region.export')) {
            $defaultBtns = array_merge($defaultBtns, ['export']);
        }

        if (Auth::user() && Auth::user()->hasPermission('region.print')) {
    ...
    protected function checkDefault($item){
        if( isset($item->is_default) && $item->is_default){
            if(Auth::user() && (Auth::user()->is_admin || Auth::user()->isSuperUser())){
                $this->editPermissions = 'region.edit';
            }else{
