...
class CommunityWiseReportsTable extends TableAbstract {
    ...
    public function __construct(DataTables $table, UrlGenerator $urlGenerator, ReportsInterface $reportsRepository) {
        ...
        $this->community = \Impiger\MasterDetail\Models\MasterDetail::where(['attribute' => 'community'])->get();
    ...
    public function query() {
        ...
        foreach($this->community as $key =>  $row){
//            $select[] = DB::Raw('(SELECT count(E_'.$key.'.community) from entrepreneurs E_'.$key.' JOIN trainees AS T ON T.entrepreneur_id = E_'.$key.'.id  where  E_'.$key.'.community = '.$row->id.') as `'.$row->slug.'`');
            $select[] = DB::Raw('(SUM( CASE WHEN entrepreneurs.community = '.$row->id.' THEN 1 ELSE 0 END )) as `'.$row->slug.'`');
        }
    ...
    public function getFilters(): array {
        ...
        Arr::forget($filters, ['district','gender','religion','candidate_type']);
        return $filters;
    }
     /**
     * @param Builder $query
