    ...
    public function boot(FormBuilder $formBuilder)
    {
        ...
        $this->app->booted(function () use ($formBuilder) {
            ...
            if (function_exists('add_shortcode')) {
                ...
                $that = $this;
                CrudHelper::declareShortCode($that, 'tnsi-startup');
            }
        });
    }

    public function addCrudStatsWidget($widgets, $widgetSettings) {
                return CrudHelper::addCrudStatsWidget('tnsi-startup',$widgets, $widgetSettings);
            }
        ...
        if (defined('THEME_OPTIONS_MODULE_SCREEN_NAME')) {
            $this->app->booted(function () {
                Theme::asset()
                    ->container('footer')
                    ->usePath(false)
                    ->add('tnsi-startup-custom-js', asset('vendor/core/plugins/tnsi-startup/js/tnsi-startup.js'), ['jquery'], [], '1.0.0');
            });
        }
    }
