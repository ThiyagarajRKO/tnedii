...
namespace Impiger\Base\Supports;
...
//        $currentUrl = URL::full();
        ...
        foreach ($links as $key => &$link) {
            ...
            if ($link['permissions'] && Auth::user() && !Auth::user()->hasAnyPermission($link['permissions'])) {
            ...
            foreach ($link['children'] as $subKey => $subMenu) {
                /* @Customized By Ramesh Esakki  - Start -*/
                if ($subMenu['permissions'] && Auth::user() && !Auth::user()->hasAnyPermission($subMenu['permissions'])) {
                    /* @Customized By Ramesh Esakki  - End -*/
    ...
    {
        $menus = $this->getAll();
        $priorities = $menus->filter(function ($item) {
            return false !== stristr($item['id'], 'cms-plugins-');
        });
        return $priorities->pluck('priority')->all();
    }
    ...
    public function getChildMenuCount($id)
    {
        $menus = $this->getAll();
        $childrens = $menus->where('id', $id)->pluck('children')->all();
        return $childrens;
    }
