class Impiger {
    constructor() {
        ...
        Impiger.initResources();
        Impiger.handleCounterUp();
        Impiger.initMediaIntegrate();
        if (ImpigerVariables && ImpigerVariables.authorized === "0") {
    ...
    static showNotice(messageType, message, messageHeader = "") {
        ...
        if (!messageHeader) {
            switch (messageType) {
                case "error":
                    messageHeader =
                        ImpigerVariables.languages.notices_msg.error;
                    break;
                case "success":
                    messageHeader =
                        ImpigerVariables.languages.notices_msg.success;
                    break;
            }
        }
    ...
    static handleError(data) {
        if (typeof data.errors !== "undefined" && !_.isArray(data.errors)) {
            Impiger.handleValidationError(data.errors);
        } else {
            if (typeof data.responseJSON !== "undefined") {
                if (typeof data.responseJSON.errors !== "undefined") {
                    if (data.status === 422) {
                        Impiger.handleValidationError(data.responseJSON.errors);
                    }
                } else if (typeof data.responseJSON.message !== "undefined") {
                    Impiger.showError(data.responseJSON.message);
                } else {
                    $.each(data.responseJSON, (index, el) => {
                        $.each(el, (key, item) => {
                            Impiger.showError(item);
                        });
                    });
                }
            } else {
                Impiger.showError(data.statusText);
            }
        }
    }
            ...
            settings = $.extend(
                {
                    ...
                    format:
                        "(%1 " +
                        ImpigerVariables.languages.system.character_remain +
                        ")",
                    pulse: true,
                    delay: 0,
                },
    ...
    static initResources() {
        ...
        Impiger.initDatePicker(".datepicker");
        if (jQuery().mCustomScrollbar) {
            Impiger.callScroll($(".list-item-checkbox"));
        }
    ...
    static initCodeEditor(id, type = "css") {
        ...
        $(".handle-tool-drag").mousedown((event) => {
            ...
            $("body")
                ...
                .on("mousemove", Impiger.onDragTool);
            $(window).on("mouseup", Impiger.onReleaseTool);
        });
    }
    ...
    static onReleaseTool() {
        $("body").off("mousemove", Impiger.onDragTool);
        $(window).off("mouseup", Impiger.onReleaseTool);
    }
