...
class rvMedia {
    constructor(selector, options) {
        ...
        let clickCallback = event =>  {
            ...
            let isImageFileType = ($current.data('action') == 'select-image' || ($current.is('.btn_select_gallery'))) ? true : false;

            MediaConfig.request_params.filter = (isImageFileType) ? 'image' : 'everything';
            MediaConfig.request_params.disableFilter = true;
            Helpers.storeConfig();
            ...
            if ($('#rv_media_body .rv-media-container').length === 0) {
                $('#rv_media_body').load(RV_MEDIA_URL.popup, data => {
                    ...
                    if(isImageFileType) {
                        $(document).find('.rv-media-container .js-rv-media-change-filter-group:first').attr('disabled', 'disabled');
                    }
                    /* Customized @Ramesh.Esakki */
...
$.fn.rvMedia = function (options) {
    ...
    MediaConfig.request_params.filter = 'image';
