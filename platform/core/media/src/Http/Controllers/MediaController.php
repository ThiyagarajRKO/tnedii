...
namespace Impiger\Media\Http\Controllers;

use Impiger\Media\Http\Resources\FileResource;
use Impiger\Media\Http\Resources\FolderResource;
...
use Impiger\Media\Services\UploadsManager;
use Impiger\Media\Supports\Zipper;
use Eloquent;
use Exception;
use File;
...
class MediaController extends Controller
{
    ...
    public function getList(Request $request)
    {
        ...
        if($user && $user->super_user != 1) {
            $paramsFile['condition'] = [
                ['media_files.user_id', '=', Auth::id()],
            ];
    
            $paramsFolder['condition'] = [
                ['media_folders.user_id', '=', Auth::id()],
            ];
        }
        /* Customized @Ramesh.Esakki */
