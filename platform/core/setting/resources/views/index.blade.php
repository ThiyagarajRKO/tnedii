            ...
            <div class="flexbox-annotated-section">
                ...
                <div class="flexbox-annotated-section-content">
                    <div class="wrapper-content pd-all-20">
                            ...
                            <div class="ui-select-wrapper">
                                <select name="locale" class="ui-select form-control select-search-full" id="locale">
                                    @php
                                        ...
                                        if (app()->environment('demo') && session('site-locale') && array_key_exists(session('site-locale'), \Impiger\Base\Supports\Language::getAvailableLocales())) {
                                            $defaultLocale = session('site-locale');
                                        }
                                    @endphp
                                    @foreach (\Impiger\Base\Supports\Language::getAvailableLocales() as $key => $locale)
                        ...
                        <div class="form-group">
                            <label class="text-title-field"
                                   for="cache_time">{{ trans('core/setting::setting.general.session_idle_check_time') }}</label>
                            <input step="0.25" type="number" class="next-input" name="session_idle_check_time" id="session_idle_check_time"
                                   value="{{ setting('session_idle_check_time', 1) }}">
                        </div>
                        ...
                        <div class="form-group">
                            <label class="text-title-field"
                                   for="locale">{{ trans('core/setting::setting.general.default_country') }}
                            </label>
                            <div class="ui-select-wrapper"><?php
                                        $defaultCountryId = DB::table('countries')->where('country_code', config('app.default_country_code'))->get('id')->pluck('id')->first();
                                        $defaultCountry = setting('default_country', $defaultCountryId);
                                        setting('default_country', $defaultCountryId);
                                        $countries =  DB::table('countries')->get();
                                    ?>
                                <select name="default_country" class="ui-select form-control select-search-full" id="default_country">
                                    @foreach ($countries as $key => $country)
                                        <option value="{{ $country->id }}" @if ($defaultCountry == $country->id) selected @endif>{{ $country->country_name }} - {{ $country->country_code }}</option>
                                    @endforeach
                                </select>
                                <svg class="svg-next-icon svg-next-icon-size-16">
                                    <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#select-chevron"></use>
                        ...
                        <div class="form-group">
                            <label class="text-title-field"
                                   for="locale">{{ trans('core/setting::setting.general.site_description') }}
                            </label>
