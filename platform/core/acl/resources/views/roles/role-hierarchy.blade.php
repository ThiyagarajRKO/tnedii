    ...
    <div class="row">
        ...
        <div class="col-md-5">
            <label class="control-label">Child {{trans('core/acl::users.role')}}</label>
            <div class="input-group">
    ...
    <div class="draggable-main">
        ...
        <div class='row'>
            <ul class="width-45 moduleListBox connected-sortable draggable-left">
                ...
                @if (isset($role->id) && !in_array($role->id, $mappedRoles))
                <li class="listItems">
            ...
            <ul class="width-45 moduleListBox connected-sortable draggable-right">
                ...
                @if (isset($role->id) && in_array($role->id, $mappedRoles))
                <li class="listItems">
                    <input type='hidden' class="form-control" name='child_roles[]' value='{{$role->id}}'>
                    <label class="control-label" style="margin: 5px;">{{ $role->name }}</label>
                </li>
                @endIf
                @endforeach
...
<script>
    $(document).ready(function () {
        $('#roleHierarchyContainer').customListBox({
            nameKey: 'child_roles[]'
        });
    });
</script>
