...
//                return view('core/acl::users.partials.role', ['item' => $item])->render();
    ...
    public function query()
    {
        ...
        $select = [
            ...
            'users.username',
            'users.email',
//            'roles.name as role_name', /*Customized by Sabari Shankar.Parthiban*/
            'roles.id as role_id',
            'users.updated_at',
            'users.created_at',
            'users.super_user',
        ];
        $select[] = DB::raw('GROUP_CONCAT(DISTINCT roles.name) as role_name');
        /*
         * @customized Sabari Shankar.Parthiban
         * It's support single role we change it in multi role
         */
...
//            ->select($select);
        $query = $model->leftJoin('role_users', 'users.id', '=', 'role_users.user_id')
            ->leftJoin('roles', 'roles.id', '=', 'role_users.role_id')
            ->select($select)->groupBy("role_users.user_id");
