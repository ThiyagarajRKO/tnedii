...
class ThemeOption
{
    ...
    public function setOption(string $key, ?string $value = ''): self
    {
        ...
        Setting::set($this->getOptionKey($key, $this->getCurrentLocaleCode(), $this->getCurrentDomainId()), $value);
     ...
     * @customized vijayaragavan ambalam
     */
    protected function getOptionKey(string $key, ?string $locale = '', ?string $domainid = ''): string
    {
        return $this->optName . '-' . setting('theme') . $locale . $domainid . '-' . $key;
    }
     ...
     * @customized vijayaragavan ambalam
     */
    protected function getCurrentDomainId(): ?string
    {
        $multidomainId = null;
        if (is_plugin_active('multidomain')) {
            $currentDomainId = app(\Impiger\Multidomain\Multidomain::class)->getCurrentAdminDomainId();
            $multidomainId = $currentDomainId ? '-' . $currentDomainId : null;
        }

        return $multidomainId;
    }
    ...
    {
        return setting()->has($this->getOptionKey($key, $this->getCurrentLocaleCode(), $this->getCurrentDomainId()));
    }
    ...
    public function getOption(string $key = '', $default = ''): ?string
    {
        ...
        $value = setting($this->getOptionKey($key, $this->getCurrentLocaleCode(), $this->getCurrentDomainId()), $default);
