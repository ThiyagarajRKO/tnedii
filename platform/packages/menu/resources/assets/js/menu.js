...
class MenuNestable {
    ...
    handleNestableMenu() {
        ...
        $(document).on(
            "click",
            ".dd-item .dd3-content a.show-item-details",
            (event) => {
        ...
        $(document).on(
            "change blur keyup",
            '.nestable-menu .item-details input[type="text"], .nestable-menu .item-details select',
            (event) => {
                event.preventDefault();
                ...
                parent
                    .find('> .dd3-content .text[data-update="' + name + '"]')
                    .text(value);
                if (value.trim() === "") {
                    parent
                        .find(
                            '> .dd3-content .text[data-update="' + name + '"]'
                        )
                        .text(old);
                }
                that.setDataItem(
                    that.$nestable.find("> ol.dd-list li.dd-item")
                );
            }
        );
        ...
        $(document).on(
            "click",
            ".box-links-for-menu .btn-add-to-menu",
            (event) => {
                event.preventDefault();
                let current = $(event.currentTarget);
                ...
                if (parent.attr("id") === "external_link") {
                    ...
                    let data_target = sanitizeHTML(
                        $("#target").find("option:selected").val()
                    );
                    let url_html =
                        '<label class="pad-bot-5"><span class="text pad-top-5 dis-inline-block" data-update="custom-url">Url</span><input type="text" data-old="' +
                        data_url +
                        '" value="' +
                        data_url +
                        '" name="custom-url"></label>';
                    html +=
                        '<li data-reference-type="' +
                        data_type +
                        '" data-reference-id="' +
                        data_reference_id +
                        '" data-title="' +
                    ...
                    html +=
                        '<span class="text float-left" data-update="title">' +
                        data_title +
                        "</span>";
                    html +=
                        ...
                        "</span>";
                    html +=
                        '<a href="#" class="show-item-details"><i class="fa fa-angle-down"></i></a>';
                    ...
                    html += url_html;
                    html +=
                        '<label class="pad-bot-5 dis-inline-block"><span class="text pad-top-5" data-update="icon-font">Icon - font</span><input type="text" name="icon-font" value="' +
                        data_font_icon +
                        '" data-old="' +
                        data_font_icon +
                    ...
                    html +=
                        '<div style="width: 228px; display: inline-block"><select name="target" id="target" data-old="' +
                        data_target +
                    ...
                    parent.find(".list-item li.active").each((index, el) => {
                        ...
                        let data_type = sanitizeHTML(
                            find_in.attr("data-reference-type")
                        );
                        let data_reference_id = sanitizeHTML(
                            find_in.attr("data-reference-id")
                        );
                        let data_title = sanitizeHTML(
                            find_in.attr("data-title")
                        );
                        ...
                        html += "</label>";
                        html +=
                            '<label class="pad-bot-5 dis-inline-block"><span class="text pad-top-5" data-update="icon-font">Icon - font</span><input type="text" name="icon-font" class="form-control"></label>';
                        html += '<label class="pad-bot-10">';
                        html +=
                            '<span class="text pad-top-5 dis-inline-block" data-update="class">CSS class</span>';
                        html +=
                        ...
                        html +=
                            '<span class="text pad-top-5 dis-inline-block" data-update="target">Target</span>';
                        html +=
                            '<div style="width: 228px; display: inline-block"><select name="target" id="target" class="form-control select-full">';
                        html +=
                ...
                $(".nestable-menu > ol.dd-list").append(
        ...
        $(document).on(
            "click",
            ".nestable-menu .item-details .btn-remove",
            (event) => {
                event.preventDefault();
                let current = $(event.currentTarget);
        ...
        $(document).on(
            "click",
            ".nestable-menu .item-details .btn-cancel",
            (event) => {
                event.preventDefault();
        ...
        $(document).on(
            "change",
            ".box-links-for-menu .list-item li input[type=checkbox]",
            (event) => {
        ...
        $(document).on("submit", ".form-save-menu", () => {
            if (that.$nestable.length < 1) {
                ...
                let nestable_obj_returned =
                    that.$nestable.nestable("serialize");
                let the_obj = that.updatePositionForSerializedObj(
                    nestable_obj_returned
                );
