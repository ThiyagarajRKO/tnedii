...
/**
 * Laravel 5 & 6 CustomCaptcha package
 *
...
class CustomCaptcha
{
    ...
    public function __construct(
        ...
        $this->hasher = $hasher;
        $this->str = $str;
        $this->characters = config('customcaptcha.characters', ['1', '2', '3', '4', '6', '7', '8', '9']);
        $this->fontsDirectory = config('customcaptcha.fontsDirectory',  dirname(__DIR__) . '/assets/fonts');
    }
    ...
    protected function configure($config)
    {
        ...
        if ($this->config->has('customcaptcha.' . $config)) {
            foreach ($this->config->get('customcaptcha.' . $config) as $key => $val) {
                if($key=='math')
                {
                    $val=filter_var(setting('custom_captcha_type'),FILTER_VALIDATE_BOOLEAN);
                }
                $this->{$key} = $val;
            }
        }
    ...
    protected function generate(): array
    {
        ...
        $this->session->put('customcaptcha', [
    ...
    {
        if (!$this->session->has('customcaptcha')) {
            return false;
        }

        $key = $this->session->get('customcaptcha.key');
        $sensitive = $this->session->get('customcaptcha.sensitive');
        $encrypt = $this->session->get('customcaptcha.encrypt');
        ...
        if ($check) {
            $this->session->remove('customcaptcha');
        }
        return $check;
    }
    ...
    /**
     * Generate customcaptcha image source
     *
     * @param string $config
     * @return string
     */
    ...
    {
        return url('customcaptcha/' . $config) . '?' . $this->str->random(8);
    }

    /**
     * Generate customcaptcha image html tag
     *
     * @param string $config
     * @param array $attrs
