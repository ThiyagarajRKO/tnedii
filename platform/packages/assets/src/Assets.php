...
class Assets
{
    ...
    /**
     * Add scripts directly to header tag.
     *
     * @param string|array $assets
     * @param string $location
     * @return $this
     ...
     */
    public function addScriptsDirectlyToHeader($assets, $location = self::ASSETS_SCRIPT_POSITION_HEADER)
    {
        foreach ((array)$assets as &$item) {
            if (!in_array($item, $this->appendedScripts[$location])) {
                $this->appendedScripts[$location][$item] = [
